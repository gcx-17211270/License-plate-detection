...
void CImgProView::OnFileOpen() 
{
	...
	MyFDlg.m_ofn.lpstrInitialDir = "C:\\Users\\32353\\Desktop\\点击刷新\\四种噪声图像";
...
////////  add processing function here  ///////////////////
	...
	Median(image, width, height, filtImg);
...
void CImgProView::Median( BYTE*image, int wid, int hei, BYTE *outImg)
{
	int i, j;
	BYTE block[9];
	int m, n;
	...
	for(i=0;i<hei;i++)
		for (j = 0; j < width; j++) {
			if (i == 0 || j == 0 || i == wid - 1 || j == hei - 1)
				...
				value = MidValueFind(9, block);
				outImg[i * wid + j ] = value;
			}
		}
...
BYTE CImgProView:: MidValueFind( int num, BYTE*d)
{
	int i, j, temp;
	int value;
	value = d[0];
